...
export function SegmentedDateStep(props: SegmentedDateStepProps) {
  ...
  return (
    <div className="processing-dialog__step processing-dialog__step--calendar-date">
      <div
        className="processing-dialog__label"
        id="processing-segmented-date-label"
      >
        {props.label}
      </div>
      <DateControl
        dateState={control.dateState}
        error={control.error}
        inputRef={control.inputRef}
        onKeyDown={control.handleKeyDown}
      />
      {control.error && (
        <div className="processing-dialog__error" role="alert">
          {control.error}
        </div>
      )}
    </div>
...
function useSegmentedDateControl(
  props: SegmentedDateStepProps,
): DateControlModel {
  const inputRef = useRef<HTMLDivElement>(null);
  const [dateState, setDateState] = useState<SegmentedCalendarDateState>(() =>
    initialDateState(props),
  );
  ...
  const applyToday = () =>
    applyTodayDate(setError, setDateState, props.onDateChange);
  const clearDate =
    props.enableClearShortcut && props.mode === "optional"
      ? () => clearOptionalDate(setError, setDateState, props.onDateChange)
      : undefined;
  const confirmDate = () => confirmDateValue(dateState, setError, props);
  const moveFocus = (direction: "h" | "l") =>
    moveDateFocus(dateState, direction, setError, setDateState);
  const handleKeyDown = (event: KeyboardEvent<HTMLDivElement>) =>
    handleDateKeyDown(event, {
      ...
      clearDate,
      confirmDate,
      enableTodayShortcut: Boolean(props.enableTodayShortcut),
      moveFocus,
      onBack: props.onBack,
    });
...
function handleDateKeyDown(
  ...
  if (event.key === "h" || event.key === "l")
    return actions.moveFocus(event.key);
  if (/^\d$/.test(event.key)) actions.applyDigit(event.key);
}

function applyDateDigit(
  state: SegmentedCalendarDateState,
  digit: string,
  setError: SetStringState,
...
function clearOptionalDate(
  setError: SetStringState,
  setState: SetDateState,
  onDateChange: (date: string) => void,
): void {
  setError("");
  setState(blankSegmentedCalendarDateState());
  onDateChange("");
}

function confirmDateValue(
  state: SegmentedCalendarDateState,
  setError: SetStringState,
  props: SegmentedDateStepProps,
): void {
  const isoDate = segmentedCalendarDateIsoValue(state);
  if (props.mode === "optional" && isBlankDateState(state))
    return props.onDateSelected(null);
  if (!isoDate) return setError(props.invalidMessage);
  props.onDateChange(isoDate);
  props.onDateSelected(isoDate);
}

function moveDateFocus(
  state: SegmentedCalendarDateState,
  direction: "h" | "l",
  setError: SetStringState,
  setState: SetDateState,
...
function DateControl({
  ...
  inputRef,
  onKeyDown,
}: DateControlProps): ReactElement {
  return (
    <div
      ref={inputRef}
      aria-invalid={error ? "true" : "false"}
      aria-labelledby="processing-segmented-date-label"
      className="processing-dialog__input processing-dialog__date-control"
      onKeyDown={onKeyDown}
      role="textbox"
      tabIndex={0}
    >
      <DateSegmentText
        active={dateState.activeSegment === "day"}
        value={dateState.day}
        width={2}
      />
      <span className="processing-dialog__date-separator">/</span>
      <DateSegmentText
        active={dateState.activeSegment === "month"}
        value={dateState.month}
        width={2}
      />
      <span className="processing-dialog__date-separator">/</span>
      <DateSegmentText
        active={dateState.activeSegment === "year"}
...
function DateSegmentText({
  active,
  value,
  width,
}: DateSegmentTextProps): ReactElement {
  const className = active
    ? "processing-dialog__date-segment processing-dialog__date-segment--active"
    : "processing-dialog__date-segment";
